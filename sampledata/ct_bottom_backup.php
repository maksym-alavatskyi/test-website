...
<?php
 ...
 <?php
	...
	if((isStage($subdomain) || isProd($subdomain)) && !is_null($myId) && (strcasecmp($myPartitonId, 'cs-stage') != 0) ) {
		if($myPartitonId == 01){
			$myPartitonId = "";
		}
		...
		<?php } else if(0 === strpos($subdomain, 'prod01-cm-ir')) { ?>
		 <script type='text/javascript'>
			var script = document.createElement("SCRIPT");
		...
		<?php }else{ ?>
		 <script type='text/javascript'>
			 console.log("Loading the ptc - partition <?=$myPartitonId?> | guid <?=$myId?>");
		 	 var script = document.createElement("SCRIPT");
			 script.src = "https://cdnssl.clicktale.net/www<?=$myPartitonId?>/ptc/<?=$myId?>.js";
			 document.body.appendChild(script);
		</script>
...
<?php } else if($myPartitonId =='cs-stage') { ?>
		<script type='text/javascript'>
